---
title: "UC Berkeley - ICLM Analysis"
author: "Rich McGowan"
...
setwd("C:/Users/rmcgowan/DESKTOP/ICLM")
library(readr)
library(tidyr)
library(dplyr)
library(reshape2)
...
GRAD_MAIN <- data.frame(ICLM_MAIN %>% filter(Career == 'GRAD',UnitsTakenPrgrss > 0))
...
#Calculate Undergraduate Average FTES based on the distinct total number of terms in the data frame (e.g. summing the Units Taken Profgress dividing by 12 then dividing by 7 regular terms-Fall 2015 thru Fall 2018-gives a 3 year average)
UGRD_FTE_THREEYRAVG <- UGRD_MAIN %>% group_by(StdntCollege,Major,CrseCollege) %>% summarize(FTES= round(sum(UnitsTakenPrgrss)/12)/numterms)
