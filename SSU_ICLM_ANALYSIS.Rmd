...
# Transform the Row Values of Course College to Column Values for the Data Frame
...
#Add "TOTAL" to the row label
for (i in 1:7) {
    Subtotals_by_College$StdntCollege[i] <-      paste(Subtotals_by_College$StdntCollege[i]," TOTAL")
}
Subtotals_by_College[,"Major"] <- " "
Subtotals_by_College <- Subtotals_by_College %>% select(StdntCollege,Major,`Arts and Humanities`,`Business and Economics`,Education,`Science & Technology`,`Social Sciences`,`University-wide`)

```
###  Add the Student College FTES Subtotals to the Data Frame
```{r Add Subtotals, echo=TRUE}

#Insert (bind_rows) the Subtotals into the Major_Course_College_Pivot data frame

bind_rows(Major_Course_College_Pivot[3,],Subtotals_by_College[1,])
```
